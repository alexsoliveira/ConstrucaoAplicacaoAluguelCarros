...
public class ProcessaLocacao
{
    ...
    public async Task Run(
        [ServiceBusTrigger("fila-locacao-auto-queue", Connection = "ServiceBusConnection")]
    ...
    {        
        ...
        try
        {
            ...
            var command = new SqlCommand(@"INSERT INTO Locacao (Nome, Email, Modelo, Ano, TempoAluguel, Data)
                    VALUES (@Nome, @Email, @Modelo, @Ano, @TempoAluguel, @Data)", connection);
            command.Parameters.AddWithValue("@Nome", rentModel.nome);
            command.Parameters.AddWithValue("@Nome", rentModel.email);
